...
function renderDraw() {
    playerDiv.textContent = 'It is a Tie!';
}
...
function checkIfCanWin() {
    ...
    const ans = [s1, s2, s3, s4, s5, s6, s7, s8].filter(line => {
        return (
...
function renderCurrentPlayer() {
    console.log('renderCurrentPlayer', turn);
    playerDiv.textContent = `${turn == 0 ? 'Your turn' : 'PC turn'}`;
}
...
function renderBoard() {
    ...
    let bReload = document.querySelector('#bReload');
    bReload.addEventListener('click', _ => {
            location.reload();
    });
